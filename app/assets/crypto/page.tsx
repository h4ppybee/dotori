...
import { CoinDonut } from "@/components/coin/CoinDonut";
import { CoinAccountDialog } from "@/components/coin/CoinAccountDialog";
import { useCoin, useCoinMutations } from "@/lib/query/use-coin";
import type { CoinHolding } from "@/lib/types";

export default function AssetsCryptoPage() {
  const { data: vm, isLoading } = useCoin();
  const { upsert } = useCoinMutations();
  const router = useRouter();
  const [addOpen, setAddOpen] = useState(false);

  async function handleAdd(input: Partial<CoinHolding> & { id?: string }) {
      ...
      <main className="min-h-screen bg-canvas">
        <div className="mx-auto w-full max-w-[480px] px-4 pt-6">
          <div className="flex flex-col items-center justify-center py-24">
            <span className="text-[15px] text-muted">코인을 불러오는 중이에요…</span>
          </div>
        </div>
      </main>
    );
        ...
        {!hasRows ? (
          <Card variant="hero" className="flex flex-col items-center text-center gap-5 py-10">
            <div className="flex flex-col gap-2">
              <p className="text-[19px] font-bold leading-[1.4] tracking-[-0.2px] text-ink">아직 코인이 없어요</p>
              <p className="text-[15px] font-normal leading-[1.6] text-body-soft max-w-[280px]">
                보유한 코인을 직접 추가해 한눈에 모아 보세요.
              </p>
            </div>
          ...
          <>
            ...
            <div className="flex gap-2">
              ...
              <Button variant="primary" onClick={() => router.push("/assets/crypto/manage")} className="flex-1">관리</Button>
            </div>
          </>
        )}
      </div>
