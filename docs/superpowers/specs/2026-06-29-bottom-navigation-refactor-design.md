...
> **개정 (2026-06-29)**: 자산 진입 시의 표현을 변경했다. 초안은 "메인 바를 유지한 채 그
> 위에 엣지투엣지 중첩 바를 쌓는" 구조였으나, 토스 증권의 실제 UX에 맞춰 **자산 진입 시
> 메인 5탭 바는 아래로 슬라이드되어 사라지고, 둥근 플로팅 바(pill)만 하단에 떠오르는**
> 방식으로 확정했다. pill은 좌우 16px 여백 + `shadows.floating` + `rounded.pill`로 떠 있는
> 알약 형태이며, ← 또는 카테고리 이탈로만 빠져나온다. 아래 본문의 "메인 바 위에 쌓인다 /
> 56+48 패딩 / 엣지투엣지" 서술은 이 개정으로 대체된다. 최종 규격은 DESIGN.md의
> `{component.asset-sub-tab-bar}`·`{component.main-tab-bar}`·`{motion.nested-bar-slide}`를
> 따른다.
...
- **AppNav**: 단일 진입점. `pathname`으로 (1) 모든 네비 숨김 여부(`/holdings/*`), (2) 중첩 바
  ...
  LockGate는 **잠금 해제된 `children` 경로에서만** `AppNav`를 렌더한다(현재 LockGate의
  `if (!locked)` 분기 위치 그대로). loading/setup/unlock 등 잠금 화면은 풀스크린이므로
  AppNav를 거치지 않고 기존대로 둔다.
...
- **중첩 바(AssetSubTabBar)**: 항상 마운트하고 노출 여부로 클래스 토글.
  ...
  - 항상 마운트하므로 **등장(slide-up)·사라짐(slide-down) 양방향 모두 애니메이션**되고
    언마운트 깜빡임이 없다.
- **위치/레이아웃**: 두 바 모두 `fixed inset-x-0`. 메인 바는 기존대로 `bottom-0`. 중첩 바는
  메인 바 바로 위에 쌓이도록 `bottom: calc(56px + env(safe-area-inset-bottom))`에 고정하고
  높이 48px. 노출 시 `translate-y-0`이면 메인 바 위에 정확히 붙고, 숨김 시 `translate-y-full`로
  메인 바 영역 아래로 내려가 가려진다. z-index는 메인 바(`z-40`)와 동일 레이어로 두되 중첩
  바를 메인 바보다 먼저(아래) 렌더해 슬라이드다운 시 메인 바에 가려지게 한다.
...
- 메인 탭 아이콘 인벤토리: **신규 4종(홈·자산·계획·가계부)** 필요, **설정·주식(차트)은 기존
  `BottomTabBar`의 SettingsIcon·PortfolioIcon 재사용**. 자산 서브탭은 라벨만으로도 충분하나
  아이콘을 둘 경우 DESIGN.md 아이콘 규칙(단순 solid/stroke)을 따른다.
...
- **lastMainTab 가드**: 메인 탭에서 자산 탭 클릭 시 직전 경로가 저장되고, 이미 `/assets/*`에
  있을 때 서브탭 간 이동에서는 `lastMainTabPath`가 갱신되지 않는지(← 복귀 대상 오염 방지)를
  명시적 케이스로 검증한다.
